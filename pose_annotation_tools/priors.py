...
def generate_aspect_ratios(dataset, num_aspect_ratios=11, visualize=True, warp_bboxes=True):
  ...
  """
    ...
    for image_data in dataset:
        ...
        bbox_xmin = np.copy(np.atleast_2d(image_data['object']['bbox']['xmin']).T)
        bbox_xmax = np.copy(np.atleast_2d(image_data['object']['bbox']['xmax']).T)
        bbox_ymin = np.copy(np.atleast_2d(image_data['object']['bbox']['ymin']).T)
        bbox_ymax = np.copy(np.atleast_2d(image_data['object']['bbox']['ymax']).T)
    ...
    i = np.isclose(X[:, 0], 5,
                   5)  # make sure the aspect ratio isn't crazy, somwhere in the range of [0, 10] should be fine
    X = X[i]
