...
def input_nodes(
  ...
  with tf.name_scope('inputs'):
    ...
    # Convert the pixel values to be in the range [0,1]
    if image_ori.dtype != tf.float32:
      image_ori = tf.image.convert_image_dtype(image_ori, dtype=tf.float32)
    # Get the images in the range [-1, 1]
    image_ori = tf.subtract(image_ori, 0.5)
    image_ori = tf.multiply(image_ori, 2.0)
    ...
    ori_images = tf.zeros([0,cfg.HEIGHT, cfg.WIDTH, 3], dtype=tf.float32)
    image_ori = tf.expand_dims(image_ori, 0)
    ori_images = tf.concat(axis=0, values=[ori_images, image_ori])
    ori_images = tf.concat(axis=0, values=[ori_images, image_ori])
    ori_images.set_shape([None, cfg.HEIGHT, cfg.WIDTH, 3])

    batched_images, batched_bboxes, batched_scores, batched_image_ids, batched_labels, batched_image_height_widths, batched_crop_bboxes, batched_ori_images = tf.train.batch(
      [cropped_images, bboxes, scores, image_ids, labels, image_height_widths, crop_bboxes,ori_images],
  ...
  return batched_images, batched_bboxes, batched_scores, batched_image_ids, batched_labels, batched_image_height_widths, batched_crop_bboxes, batched_ori_images
