...
def hourglass(input, num_branches, input_channels, output_channels, num_res_modules=1, scope=None, reuse=None):
  ...
  with tf.compat.v1.variable_scope(scope, "hourglass", [input], reuse=reuse):
    ...
    with tf.compat.v1.variable_scope("lower_branch"):
      ...
      up2 = tf.compat.v1.image.resize_nearest_neighbor(images=low3, size=[low3_height * 2, low3_width * 2], align_corners=False)
...
def build_hg(input, resid, num_parts, stack_range, num_features=256, num_stacks=8, num_res_modules=1, reuse=None, scope='HourGlass'):
