...
def errorsAPImpact( coco_analyze, saveDir ):
    ...
    areaRngs    = [[32**2,1e5**2],[96**2,1e5**2]]
    # areaRngs    = [[32**2,1e5**2],[32**2,96**2],[96**2,1e5**2]]
    areaRngLbls = ['all','large']
    ...
    for eind, err in enumerate(err_types):
        ...
        errs = [np.mean([error_perf[err,'all'][i],error_perf[err,'large'][i]]) for i in xrange(len(oksThrs))]
        stds = [np.std([error_perf[err,'all'][i],error_perf[err,'large'][i]]) for i in xrange(len(oksThrs))]
