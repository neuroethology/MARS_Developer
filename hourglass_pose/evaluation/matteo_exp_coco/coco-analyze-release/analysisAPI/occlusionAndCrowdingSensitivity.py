...
def occlusionAndCrowdingSensitivity( coco_analyze, oks, saveDir ):
    ...
    IOU_FOR_OVERLAP = .1
    overlap_groups  = [[0],[1,2],[3,4,5,6,7]]
    # overlap_groups  = [[0],[1,2],[3,4,5,6,7,8]]
    num_kpt_groups  = [[1,2,3],[4,5],[6,7]]
    ...
    for eind,e in enumerate(err_types[1:]):
        ...
        ax = fig.add_subplot(111)
        ax.set_aspect(1)
        res = ax.imshow(err_mat[e]/(total_keypoints+np.spacing(1)), cmap=cmaps[eind], interpolation='nearest')

        width, height = err_mat[e].shape
        for x in xrange(width):
            for y in xrange(height):
                ax.annotate("%.1f"%(100*err_mat[e][x,y]/float(total_keypoints[x,y]+np.spacing(1))), xy=(y, x),
                            horizontalalignment='center',
                            verticalalignment='center',fontsize=20)
