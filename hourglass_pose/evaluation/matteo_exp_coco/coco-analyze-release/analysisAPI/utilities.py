...
"""
def sigmas_len(dts):
    kpt = dts[0]['keypoints'][0::3] if isinstance(dts,list) else dts[0::3]
    ...
    if len(kpt)>7:
        ...
        sigmas= np.array([0.02349794, 0.02573029, 0.02574004, 0.02459145, 0.03146337, 0.03146769,0.02510474])
    return sigmas
...
skeleton  = [[1,2],[1,3],[2,3],[4,5],[4,6],[5,7],[6,7],[4,8],[4,9],[5,10],[6,11]]
colors    = {(0,1): '#cd87ff', (0,2): '#cd87ff', (1,2): '#cd87ff',(3,4): '#74c8f9',
             (3,5): '#74c8f9', (4,6): '#feff95', (5,6):  '#feff95',
             (3,7): '#74c8f9',(3,8): '#74c8f9', (4,9):'#feff95', (5,10):'#feff95'}
...
def show_dets(coco_dts, coco_gts, img_info, save_path=None):
    if len(coco_dts) == 0 and len(coco_gts)==0:
        return 0
    sigmas = sigmas_len(coco_dts if coco_dts!=[] else coco_gts)
    ...
    for ann in coco_dts:
        ...
        for kk in xrange(len(sigmas)):
            kkp= [1,4,7,9] if len(sigmas)>7 else [1,4]
            kko= [2,5,8,10] if len(sigmas)>7 else [2,5]
            if kk in kkp:
