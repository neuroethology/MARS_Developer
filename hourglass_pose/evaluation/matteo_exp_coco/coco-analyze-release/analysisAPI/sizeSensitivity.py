...
def sizeSensitivity( coco_analyze, oks, saveDir ):
    ...
    for i,a in enumerate(areaRngs[:-1]):
        if areaRngLbls[i] in size_index.keys():
    ...
    x = [1,2,3,4]
    y = [len(size_index['medium']) if 'medium' in size_index.keys() else 0,
         len(size_index['large']) if 'large' in size_index.keys() else 0,
         len(size_index['xlarge']) if 'xlarge' in size_index.keys() else 0,
         len(size_index['xxlarge']) if 'xxlarge' in size_index.keys() else 0]
