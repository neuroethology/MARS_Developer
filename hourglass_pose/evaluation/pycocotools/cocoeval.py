...
class COCOeval:
	...
	if len(gts[0]['keypoints'][0::3])>7:
	    ...
	    #print('<{}:{}>Using front sigmas'.format(__author__,__version__))
	else:
	    sigmas= np.array([0.02349794, 0.02573029, 0.02574004, 0.02459145, 0.03146337, 0.03146769, 0.02510474]) #top
	    #print('<{}:{}>Using top sigmas'.format(__author__,__version__))
    ...
    def summarize(self, verbose=False):
        ...
        def _summarizeKps_verbose():
            ...
            for t in self.params.iouThrs:
                cur_stat += 1
                stats[cur_stat] = _summarize(1, maxDets=self.params.maxDets[-1], iouThr=t, areaRng=mainAreaRng)

            if len(self.params.iouThrs) > 1:
                stats[cur_stat+1] = _summarize(1, maxDets=self.params.maxDets[-1])
                stats[cur_stat+2] = _summarize(1, maxDets=self.params.maxDets[-1], areaRng='medium')
                stats[cur_stat+3] = _summarize(1, maxDets=self.params.maxDets[-1], areaRng='large')
                cur_stat += 3

            for t in self.params.iouThrs:
                cur_stat += 1
                stats[cur_stat] = _summarize(0, maxDets=self.params.maxDets[-1], iouThr=t, areaRng=mainAreaRng)

            if len(self.params.iouThrs) > 1:
                stats[cur_stat+1] = _summarize(0, maxDets=self.params.maxDets[-1])
                stats[cur_stat+2] = _summarize(0, maxDets=self.params.maxDets[-1], areaRng='medium')
                stats[cur_stat+3] = _summarize(0, maxDets=self.params.maxDets[-1], areaRng='large')
