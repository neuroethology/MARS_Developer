...
from hourglass_pose.evaluation import eval_inputs as inputs
...
def process_tfrecord(project, pose_model_names=None, max_iterations=0, show_heatmaps=False,
                     show_layer_heatmaps=False, prep_cocoEval=True):
    ...
    with graph.as_default():
        ...
        with session.as_default():
            ...
            if prep_cocoEval:
                cocodata = {'gt_keypoints': gt_dataset, 'pred_keypoints': pred_annotations, 'view': view,
                            'partNames': partNames}
...
def parse_args():
    ...
    parser.add_argument('--checkpoint_path', dest='checkpoint_path',
                        help='path to directory where the checkpoint files are stored. The latest model will be tested against.',
                        type=str,
                        required=True)
