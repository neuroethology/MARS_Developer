...
def get_err_btw_kps(gt,pd,x_scale,y_scale,ind,num_points):
    medians = gt['med']
    gt_X = medians[1, ind]*x_scale
    gt_Y = medians[0, ind]*y_scale
...
def annotations_to_error(annotation_dict,x_scale,y_scale,ind):
    raw_X = annotation_dict['Y'][:,ind]
    raw_Y = annotation_dict['X'][:,ind]
...
def get_perfs(res_file,amt_file,savedir, view):
    ...
    for i in range(len(pd_b)):
        ...
        b_err.append(annotations_to_error(black_annotations,w,h,ind))# for each annotator x # for each k
        w_err.append(annotations_to_error(white_annotations,w,h,ind))

        #amt vs pred
        pd_b_err.append(get_err_btw_kps(black_annotations,black_pd,w,h,ind,num_points))
        pd_w_err.append(get_err_btw_kps(white_annotations,white_pd,w,h,ind,num_points))
...
if __name__ == '__main__':
    ...
    get_perfs(args.res_path,args.amt_path, args.savedir,args.view)
