...
def extract_features(project, progress_bar_sig='', sets_to_process = ['train', 'test', 'val'], n_jobs=1, verbose=0):
    config_fid = os.path.join(project, 'project_config.yaml')
    with open(config_fid) as f:
        cfg = yaml.load(f, Loader=yaml.FullLoader)
    ...
    joblib.Parallel(n_jobs=n_jobs, verbose=verbose)(joblib.delayed(extract_features_single_set)(project, cfg, clf_params, key) for key in sets_to_process)
