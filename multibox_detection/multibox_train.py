...
def init_finetune_model(logdir: str, rcnn_path:str = '', 
                        pretrained_model_path:str=None, fine_tune:bool=False, 
                        original_inception_vars:list=None, use_moving_averages=False,
                        restore_moving_averages=False, ema=None):
    '''
    Args:
        logdir : location of where we will be storing checkpoint files.
        rcnn_path : TFhub path for rcnn vectorization network
...
def get_init_function(logdir:str, pretrained_model_path:str, fine_tune:bool, 
                      original_inception_vars:list=None, use_moving_averages=False,
                      restore_moving_averages=False, ema=None):
    """
    Args:
    ...
    Returns:
        Pretrained keras.Model instance or None.
    ...
    # return the latest checkpoint if the path is a directory.
    # otherwise just return the path to the checkpoint that was given
    if path.isdir(pretrained_model_path):
    ...
    if fine_tune:
        ...
        variables_to_restore = Model.load_weights
    ...
    if restore_moving_averages:
        ...
        if use_moving_averages:
            ...
            normal_saver = tf.train.Checkpoint()
            ema_saver = tf.train.Checkpoint()
...
def train(tfrecords_train:list, tfrecords_val, bbox_priors, logdir:str, cfg:dict, pretrained_model_path:str=None, fine_tune:bool=False,
          trainable_scopes=None, use_moving_averages=False, restore_moving_averages=False, debug_output=False,
          resnet_model:str='https://tfhub.dev/google/imagenet/inception_resnet_v2/feature_vector/5'):
    """
    Args:
        tfrecords_train (list): list of training tfrecord files
        tfrecords_val (list):   list of validation tfrecord files
        bbox_priors (np.array): bounding box priors for training
        logdir (str)
        cfg (EasyDict)
    ...
    image_inputs = Input(shape=(299,299,3))
    resnet_layers = hub.KerasLayer(resnet_model)
    mdl = Model() # create keras model
