  ...
  """
    ...
    for b in range(batch_size):
        ...
        for j in range(num_gt_bboxes[b]):
            C[:, j] = (alpha / 2.) * (np.linalg.norm(locations[offset:offset + num_predictions] - gt_bboxes[b][j],
                                                     axis=1)) ** 2 - log_confidences[
                                                                     offset:offset + num_predictions] + log_one_minus_confidences[
                                                                                                        offset:offset + num_predictions]
...
def compute_loss(locations, confidences, batched_bboxes, batched_num_bboxes, bbox_priors, location_loss_alpha, batch_size):
    ...
    matching, stacked_gt_bboxes = tf.numpy_function(compute_assignments, params, [tf.int32, tf.float32],
    ...
    confidence_loss = -1. * tf.reduce_sum(tf.math.log(matched_confidences)) - tf.reduce_sum(
        tf.math.log((1. - unmatched_confidences) + SMALL_EPSILON))
...
def add_loss(locations, confidences, batched_bboxes, batched_num_bboxes, bbox_priors, location_loss_alpha):
    ...
    with tf.name_scope("loss"):
        location_loss, confidence_loss = compute_loss(**locals())
