...
def export_csv(csv_exp_filename, csv_imp_filename='',
    ...
    if PARAM_DETRLSWAP:
        ...
        print(f"\tVerification complete. Detected {errors}, errors on {frame_errors} frames ({(100*float(errors)/float(n_frames*4))} / {str(100*float(frame_errors)/float(n_frames))}% ).")
