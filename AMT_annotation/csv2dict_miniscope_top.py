...
if not os.path.exists('../tf_dataset_detection/top_miniscope/'):
    os.makedirs('../tf_dataset_detection/top_miniscope/')

path = '/PATH_TO_FRAMES_IMGS/'


#parse csv file to extract data
csv_file = 'frames_miniscope_top.csv'
f = open(csv_file)
    ...
    for w in range(N_workers):
        ...
        ann = {
            ...
            'B':{'type': list([t[10] for t in tmp_w[:9]]),
                 ...
                 'xy_tail': np.array([t[11:13] for t in tmp_w[:9]]).astype(float)[7:9]},
        ...
        if len(np.array([t[11] for t in tmp_w[:9]]).astype(float))==9:
            XB.append(np.array([t[11] for t in tmp_w[:9]]).astype(float))
        if len(np.array([t[12] for t in tmp_w[:9]]).astype(float))==9:
